<script>
	...
	$: filteredData = data.filter(row => selectedLines.any(line => row.lines.includes(line)));
	...
	$: xScale = d3.scaleLinear()
			.domain([0, d3.max(data, d => d.density) || 1]) // Avoid empty data crash
			.range([0, width]);
...
<div class="container">
	<svg viewBox="0 0 {width} {barHeight * data.length}">
